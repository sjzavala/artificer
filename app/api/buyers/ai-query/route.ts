...
import { AiQueryError, AI_QUERY_MAX_LENGTH, translateQuery } from '@/lib/buyers/ai';
...
/**
 * Natural language search over the pipeline.
 *
 ...
 * a broker should be able to see that "at least $2M" became `minEquity =
 * 2000000` and correct it, rather than trusting a list of names that arrived
 * from nowhere.
 */
export async function POST(request: NextRequest) {
  ...
  const limit = rateLimit(`buyer-ai-query:${session}`, { limit: 20, windowMs: 60_000 });
  if (!limit.allowed) {
  ...
  if (!text) {
    return NextResponse.json({ error: 'Describe the buyers you are looking for.' }, { status: 400 });
  }
  ...
  try {
    ...
    console.error('[artificer] buyer ai-query failed', error);
  ...
  console.log(
    `[artificer] buyer ai-query model=${model} in=${inputTokens} out=${outputTokens} ` +
      `q="${query.q}" status=${query.status ?? '-'} source=${query.capitalSource ?? '-'} ` +
      `market=${query.market ?? '-'} type=${query.propertyType ?? '-'} ` +
      `minEquity=${query.minEquity ?? '-'} within=${query.identifyWithinDays ?? '-'} sortBy=${query.sortBy}`,
  );

  try {
    const results = await getBuyerRepo().search(query);
    return NextResponse.json({
      ...
      filters: {
        q: query.q,
        status: query.status ?? '',
        capitalSource: query.capitalSource ?? '',
        market: query.market ?? '',
        propertyType: query.propertyType ?? '',
        minGuarantor: query.minGuarantor ?? '',
        minEquity: query.minEquity === null ? '' : String(query.minEquity),
        identifyWithinDays: query.identifyWithinDays === null ? '' : String(query.identifyWithinDays),
        sortBy: query.sortBy,
      },
      interpretation,
      results,
